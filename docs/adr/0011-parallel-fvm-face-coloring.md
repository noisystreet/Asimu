...
**scatter 仍串行（v0.x / P8 过渡）**：主 crate `unsafe_code = forbid`，禁止并行写 `&mut [f64]`。**v1.0** 起并行 scatter 原语下沉 `exec`（`ParallelUnsafeAtomics` 等），见 [ADR 0013](0013-exec-parallel-scatter-execution-context.md)；`discretization` 仅提供通量 kernel，不直接 `rayon`/unsafe。

### 3. Cargo feature `parallel-fvm`

```toml
parallel-fvm = ["dep:rayon"]
default = ["parallel-fvm"]   # 2026-06-07 起默认启用
```

- **2026-06-07 修订**：dual_ellipsoid（221 万单元 / 475 万内面 / 9 色）Chrome trace 显示内面 flux 与 IDWLS 占 LU-SGS RHS 主导；9 色桶平均 ~52 万面/桶，并行度足够。`make check` / CI 与 `default` features 均含 `parallel-fvm`。
...
| `parallel_idw_lsq_accumulate_matches_face_serial` | IDWLS 单元并行 vs 面循环串行 |
| `lsq_rhs_incidence_covers_all_interior_faces` | 单元–面关联完整性 |

Makefile：`make check` 默认含 `io-vtk,parallel-fvm`；`make check-parallel-fvm` 为别名。
...
| golden 测试（粘性 + 无粘） | 已实现 |
| **`parallel-fvm` 默认启用** | **2026-06-07**（dual_ellipsoid trace） |
| **IDWLS `accumulate_rhs` 单元并行** | **2026-06-07**（P0：`LsqRhsCellIncidence` + 单元 `rayon`） |
| **`exec` 并行 scatter + `ExecutionContext`** | **已实现**（[ADR 0013](0013-exec-parallel-scatter-execution-context.md) E0–E3/E5，2026-06-08） |
| `discretization` 直接 `rayon` | **已移除**（E2；经 `exec::parallel`） |
