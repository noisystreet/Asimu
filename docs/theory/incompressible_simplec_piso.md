...
修正，并把 `max|div(u_corr)|` 与 \(\max|A_u u^*-rhs_u|\) 写入残差历史。设置 `time.tolerance` 时，连续性与动量残差须同时满足阈值才标记收敛；未设置时仅执行固定 `max_steps`，`simplec_converged=false` 表示没有收敛判据。若残差出现非有限值，runner 立即返回求解器错误；输出字段使用最后一次修正后的场。
