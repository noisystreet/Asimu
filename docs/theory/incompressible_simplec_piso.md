...
欠松弛按 (8) 后的规则修改对角与 RHS。动量边界面贡献来自 `BoundarySet`：速度入口、无滑移/动壁按 Dirichlet 速度加入扩散源项与入流迎风源项；压力出口按零梯度速度使用 owner 外推；对称/滑移壁去除法向通量。
...
runner 已提供 SIMPLEC 外层循环：`time.max_steps` 作为最大外层迭代数，
...
| I1 runner 诊断闭环 | `case/incompressible_3d.rs` | **已实现：初始化、\(max|div(u)|\)、\(max|div(u^*)|\)、动量预测三分量 GMRES 求解诊断、压力校正 GMRES 求解、一次 \(p,\mathbf{u}\) skeleton 修正诊断、CGNS 输出** |
