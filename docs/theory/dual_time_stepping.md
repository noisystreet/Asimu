...
> 模块：`src/solver/time/`、`src/solver/compressible/unstructured_*` · 版本：v1.x · 状态：**规划（实施路线图）**
...
asimu 非结构路径已实现 DTS 的**内层内核**（伪时间 + LTS + LU-SGS），但缺少：

1. 物理时间**存储项** \((U-U^n)/\Delta t_{\mathrm{phys}}\)；
2. **内外双层循环**（物理步冻结 \(U^n\)，内层伪时间迭代）；
3. 隐式更新分母对 \(1/\Delta t_{\mathrm{phys}}\) 的扩展；
4. （可选）非结构 matrix-free GMRES + DTS；
5. **自 P0 起**通过 `ComputeFloat`（`f32` / `f64`）单态化实现全路径，与 ADR [0016](../adr/0016-runtime-compute-precision.md) / [0018](../adr/0018-unstructured-compute-backend.md) 对齐，为 **GPU（CUDA f32）** 预留同等接口，禁止先写 f64 专有再事后移植。
...
式 (1) 中 \(\mathbf{R}_i\) 已含 \(1/V_i\)。存储项应写入残差为：

\[
\mathbf{R}_{\mathrm{eff},i}
= \mathbf{R}_i(\mathbf{U})
- \frac{\mathbf{U}_i - \mathbf{U}^n_i}{V_i\,\Delta t_{\mathrm{phys}}}.
\tag{4}
\]

实现时在 `assemble_unstructured_typed_rhs` **之后**叠加，**不修改** `discretization` 通量内核。
...
2. **存储项**：优先在 device 上对 `residual` 做 \(\mathbf{R}_{\mathrm{eff}} \mathrel{+}= -(\mathbf{U}-\mathbf{U}^n)/(V\,\Delta t_{\mathrm{phys}})\)；若步内暂缺 kernel，须在 validate 中明确降级（例如该组合报错），**不得**静默用 f64 算存储项再 cast 回 f32；
...
#### 3.5.3 Validate 能力矩阵（规划）

`case::validate` 在启用 `scheme = "dual_time"` 时须校验（实现期定案）：

| 组合 | 首版预期 |
|------|----------|
| `f64` + CPU | **必须**（P2–P3 基准） |
| `f32` + CPU | **必须**（与 f64 对照 smoke） |
| `f32` + CUDA | **P3b**（LU-SGS 内层；须 `local_time_step = true`） |
...
| **P3b** | CUDA f32 DTS | device `U^n` 快照 + 存储项 kernel（或 staged 方案 + validate 定案） | **f32 + CUDA** | `#[ignore=gpu]` smoke；对齐 `dual_ellipsoid/case_cuda_lusgs_f32.toml` |
...
| `unstructured_cuda_prepare_f32` | P3b：device 存储项 / `U^n` 缓冲 |
| `exec` / CUDA pipeline | P3b：存储项 kernel；步间 device 驻留契约 |
...
[time]
mode = "transient"
scheme = "dual_time"       # 规划；当前解析器未启用
dt = 1.0e-4                # Δt_phys
cfl = 5.0                  # 伪时间 CFL（内层）
local_time_step = true     # 伪时间 LTS（须 true）
max_steps = 500            # 物理步上限
max_inner_steps = 30
inner_tolerance = -3.0     # log₁₀(R_eff,rms) 阈值（规划字段名；f32 默认须放宽）
...
| 式 / 步骤 | 规划代码位置 | 状态 |
|-----------|--------------|------|
| (4) 存储项 | `solver/time/dual_time.rs::add_physical_storage_residual<T>` | 规划 |
| (6) 扩展 LU-SGS | `field` LUSGS 更新 + `lu_sgs_sweep_unstructured_*`（f32/f64） | 规划 |
| 内外循环 | `unstructured_driver_typed.rs`（`run_*::<T>`） | 规划 |
| CUDA 存储项 / \(U^n\) | `unstructured_cuda_prepare_f32`、exec CUDA kernel | 规划（P3b） |
| (8) GMRES+DTS | `gmres_implicit_3d.rs` 抽象 + unstructured RHS | 规划 |
| 配置解析 | `io/case.rs`、`DualTimeConfig::parse`、`validate` 精度矩阵 | 规划 |
...
| 算例 | 用途 |
|------|------|
| `tests/benchmarks/unstructured_freestream/` | P2 内层收敛 smoke |
| `tests/benchmarks/sod_1d/`（非结构 `nz=1` 扩展） | P3 激波对比 RK4 |
