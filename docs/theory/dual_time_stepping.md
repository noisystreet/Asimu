...
| CUDA f32 非结构双时间步 | **已实现（P3b）** | `backend = cuda` + `scheme = dual_time` + `compute_precision = "f32"`；`lusgs_sweep = false` |
...
| `f32` + CUDA | **已实现**（LU-SGS 对角内层；须 `local_time_step = true`、`lusgs_sweep = false`） |
...
| (6) 扩展 LU-SGS | `field` LUSGS 更新 + `lu_sgs_sweep_unstructured_*`（f32/f64）；CUDA 对角 `lusgs_diagonal_f32` | **已实现**（扫掠仍 CPU） |
