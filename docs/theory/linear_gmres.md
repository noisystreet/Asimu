...
做一次块 LU-SGS 双扫。Navier-Stokes case 会把粘性/热传导的抛物谱半径加入对角块，并在内部面加入动量对角扩散与 **应力张量诱导的动量交叉近邻块**（\(c_\mathrm{shear}\,n_i n_j,\ i\neq j\)，\(c_\mathrm{shear}=\sigma_\mathrm{par}\nu\)）；密度分量不叠加粘性扩散，动量对角使用 \(4\nu/3\)，能量分量使用 \(\lambda/(\rho c_v)=\gamma\nu/\mathrm{Pr}\)。边界面的抛物时间尺度进入对角块，用于覆盖壁面小尺度刚性；热传导/粘性做功的完整能量交叉项与 IDWLS 远邻梯度仍未耦合。后扫邻居块贡献乘 `[time].lusgs_sweep_backward_damping`（默认 0.5），与标量 `lu_sgs` 双扫一致。
...
- **降低 `block_lusgs` 构造成本**：…**已实现**一阶 Roe / Hanel–Van Leer 面通量解析 Jacobian；边界面 ghost 对 wall/symmetry/farfield（含 Riemann 混合）及 **亚声速 outlet** 解析 \(\partial U_g/\partial U\)；亚声速 inlet / 热流壁 / 钳制区仍数值 ghost Jacobian。块双扫后扫已接入 `lusgs_sweep_backward_damping`。
...
4. **更完整的粘性块**：当前已用分量化粘性扩散率区分密度、动量与能量，内面近邻块含动量应力交叉项 \(c_\mathrm{shear} n_i n_j\)，边界面小尺度加入对角；后续可补热传导/粘性做功能量交叉项、热流壁解析 ghost，以及与 IDWLS 梯度样本一致的远邻耦合。完整粘性块应先有小网格 Jacobian-vector 对照测试。
