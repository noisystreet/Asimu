...
| `cell_spectral_radius_unstructured_f32` | 非结构 f32 单元谱半径 \(\sigma_i\)，返回 `Vec<f32>`（面循环 f32；单元 f64 累加后落盘） |
...
| `lu_sgs_sweep_unstructured_f32` | 非结构 f32 LU-SGS 双扫（`lusgs_couplings_f32`；source/耦合差分/正性限制全 f32） |
