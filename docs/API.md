...
| `scatter_viscous_boundary_f32` | 粘性边界面 f32 scatter（原生 `+=`，无 `add_mul_real`） |
...
| `lu_sgs_common` | LU-SGS 双扫共用稳定化（线搜索、对角回退、正性限制）；f32 非结构扫掠热路径含 `residual_cell_vector_f32` / `conserved_vector_f32` |
| `lu_sgs_sweep_unstructured_f32` | 非结构 f32 LU-SGS 双扫（`lusgs_couplings_f32`；source/耦合差分全 f32） |
