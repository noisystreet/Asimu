...
| `GmresPreconditionerKind` | `ScalarDiagonal` / `CellBlockDiagonal` / `LusgsSweep` / `BlockLusgs`，对应 `[time] gmres_preconditioner` |
| `CaseTimeConfig::resolved_gmres_config` | 解析 `[time] gmres_tolerance` / `gmres_max_iters` / `gmres_restart`（仅 `scheme = gmres`）为 `GmresImplicitConfig` |
