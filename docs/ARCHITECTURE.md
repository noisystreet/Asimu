...
| `cuda` | NVIDIA CUDA（`cudarc`；[ADR 0017](adr/0017-gpu-cuda-cudarc-multi-backend.md)） |
...
| **v1.3.x** | MCP Prompts；`.cursor/mcp.json.example` | **`cuda`**（[ADR 0017](adr/0017-gpu-cuda-cudarc-multi-backend.md) G1–G2）；MCP 与 VTK 输出联动 | 可选 `--profile` flamegraph |
