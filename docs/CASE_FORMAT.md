...
| `backend` | string | `cpu`（默认）或 `cuda`。`cuda` 须以 `--features cuda` 编译，并在 Validate 阶段校验能力矩阵（ADR 0017）；非结构 f32 支持一阶 Euler/RK4、`lu_sgs`（含 `lusgs_sweep`）、`dual_time` |
...
| 时间推进 | `euler`、`rk4`；`lu_sgs`（对角或 `lusgs_sweep`，须 `local_time_step = true`）；`gmres`（matrix-free，须 `local_time_step = true`；非结构 CPU f64 支持 `scalar_diagonal` / `cell_block_diagonal` / `lusgs_sweep` / `block_lusgs` 预条件；块预条件须 `reconstruction = first_order`）；`block_lusgs`（非结构 CPU f64 块隐式伪时间，每步一次对称双扫；须 `local_time_step = true` 与 `reconstruction = first_order`）；非结构 `dual_time`（CPU f32/f64；CUDA 仅 f32） |
| 粘性 / NS | 非结构 3D：`[physics.viscous]` 与 `[navier_stokes]`（梯度/通量仍为 mixed `f64`） |
| 隐式限制 | 结构化 CUDA 不支持 `lusgs_sweep = true`；非结构 CUDA f32 支持 `lusgs_sweep`（device 双扫 + host stabilize）；非结构 CUDA 不支持 `gmres` |
...
# scheme = "rk4"      # rk4（默认）| euler | lu_sgs | gmres | block_lusgs | simplec（不可压缩稳态）| piso（不可压缩瞬态）
...
# gmres_preconditioner = "scalar_diagonal" # scalar_diagonal | cell_block_diagonal | lusgs_sweep | block_lusgs
# gmres_tolerance = 1.0e-6      # 可选；每步 GMRES 线性求解相对残差容差（预条件残差范数）
# gmres_max_iters = 60          # 可选；每步 GMRES 最大迭代次数
# gmres_restart = 20            # 可选；GMRES restart 长度
...
每个 GMRES 步在 `debug` 级别输出日志事件 `GMRES 隐式步诊断`，含 profiling 字段：`profile_compute_dt_ms`、`profile_preconditioner_build_ms`、`profile_linear_solve_ms`、`profile_line_search_ms`、`profile_step_total_ms` 与 `log10_residual`（步初 \(\|R(U^0)\|\)，与 `profile_base_residual_ms` 同源装配）。

| 字段 | 说明 |
|------|------|
| `mode` | `steady` 稳态伪时间推进；`transient` 瞬态物理时间推进 |
| `scheme` | 可选；`rk4`（默认）、`euler`、`lu_sgs`、`gmres`、`block_lusgs`（非结构 CPU f64 块隐式伪时间，每步一次 5×5 对称双扫；须 `local_time_step = true` 与 `reconstruction = first_order`）、`dual_time`（非结构可压缩瞬态隐式，CPU f32/f64；CUDA 仅 f32，见 [theory/dual_time_stepping.md](theory/dual_time_stepping.md) §3.5）、`simplec` 或 `piso`；`simplec` / `piso` 仅用于不可压缩 pressure-velocity 路径。**3D 可压缩局部时间步均用 Blazek §6.1.4/§9.1** \(\Delta t_i=\mathrm{CFL}/\sigma_i\)，\(\sigma_i=(\Lambda_i^c+C_v\Lambda_i^v)/V_i\) |
| `local_time_step` | 可选；`true` 时逐单元 CFL 时间步（稳态加速；`lu_sgs` / `gmres` / `block_lusgs` **必须**为 `true`） |
...
| `gmres_preconditioner` | 可选；`scalar_diagonal`（默认）、`cell_block_diagonal`（每单元 5×5 局部无粘 Jacobian 块；NS case 叠加粘性抛物对角谱半径）、`lusgs_sweep`（非结构 f64：标量谱半径线性 LU-SGS 双扫，系数取自当前步原始变量与 `[time]` LU-SGS 参数）或 `block_lusgs`（非结构 f64：一阶无粘 5×5 面块 Jacobian 的块 LU-SGS 双扫；NS case 叠加分量化粘性抛物对角与内面近邻近似，密度不加粘性扩散，动量/能量分别使用 \(4\nu/3\) 与 \(\gamma\nu/\mathrm{Pr}\)） |
...
| `low_mach_preconditioning` | 可选；启用低马赫预处理（P1–P2 非结构可压缩 CPU） |
| `low_mach_mach_cutoff` | 可选；低马赫截止 \(M_\text{cut}\in(0,1]\)，默认 0.1 |
| `low_mach_max_mach` | 可选；向常规可压缩谱半径退化的上界 \(M_\text{max}\)，默认 0.3，须 \(M_\text{cut}<M_\text{max}\le 1\) |
| `low_mach_blend` | 可选；`smooth`（默认）或 `hard_cut` |
| `low_mach_jacobian` | 可选；`true` 启用预处理 Roe 面 Jacobian；`lu_sgs`+`lusgs_sweep` 时走块双扫（须 `f64`、`first_order`） |

**`scheme = "dual_time"` 专用字段**：`max_inner_steps`（每物理步伪时间迭代上限）、`inner_tolerance`（内层 \(\log_{10}\|R_{\mathrm{eff}}\|_{\mathrm{rms}}\) 早停阈值）。此时 `max_steps` 计物理步，`cfl` 用于伪时间 LTS；须 `local_time_step = true` 且设正数 `dt`（\(\Delta t_{\mathrm{phys}}\)）。**须**与 `[numerics].compute_precision = "f64"|"f32"` 及 `backend = cpu|cuda` 组合使用：f32/f64 双精度 CPU 路径同等支持；`backend = cuda` 仅 f32、非结构 3D、`reconstruction = first_order`（见 [theory/dual_time_stepping.md](theory/dual_time_stepping.md) §3.5）。详见同文档 §5.2。

**低马赫预处理（P1–P4）**：字段与语义见 [theory/low_mach_preconditioning.md](theory/low_mach_preconditioning.md)。当前已支持非结构可压缩 CPU 路径；`backend = "cuda"` 暂不支持。
...
配置 `residual_csv` 时，算例结束会写出残差 CSV，并**默认**在同目录生成同名 PNG 曲线图（如 `residual.csv` → `residual.png`）；可用 `residual_plot = "custom.png"` 覆盖输出路径。需本机 `python3` 与 `matplotlib`；缺失时仅告警，不影响 CSV 写出。可压缩多步路径在 `solution_every` 间隔也会同步刷新 CSV 与曲线图。不可压缩 SIMPLEC/PISO 会在最终输出时写出逐步 pressure-velocity residual CSV；同时配置 `solution_cgns` 与 `solution_every` 时，还会写出 `flow_step000100.cgns` 形式的间隔快照。
