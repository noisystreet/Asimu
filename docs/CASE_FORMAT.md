...
求解 \((D_{\Delta t}-J_R)\Delta U=R(U)\)，默认使用 LU-SGS 标量对角预条件器，并在有限差分扰动与更新守恒量前做正性限制。`gmres_preconditioner = "cell_block_diagonal"` 可切换为每单元 5×5 局部块对角预条件器（更强耦合，构造成本更高）。
...
| `gmres_preconditioner` | 可选；`scalar_diagonal`（默认）或 `cell_block_diagonal`（每单元 5×5 局部块） |
