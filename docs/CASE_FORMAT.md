...
| 时间推进 | `euler`、`rk4`；`lu_sgs`（对角或 `lusgs_sweep`，须 `local_time_step = true`）；`gmres`（matrix-free，须 `local_time_step = true`；非结构 CPU f64 支持 `scalar_diagonal` / `cell_block_diagonal` / `lusgs_sweep`；块预条件须 `reconstruction = first_order`）；非结构 `dual_time`（CPU f32/f64；CUDA 仅 f32） |
...
# gmres_preconditioner = "scalar_diagonal" # scalar_diagonal | cell_block_diagonal | lusgs_sweep
...
| `gmres_preconditioner` | 可选；`scalar_diagonal`（默认）、`cell_block_diagonal`（每单元 5×5 局部无粘 Jacobian 块）或 `lusgs_sweep`（非结构 f64：线性 LU-SGS 双扫，系数取自当前步原始变量与 `[time]` LU-SGS 参数） |
