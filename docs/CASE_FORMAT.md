...
| `scheme` | 可选；`rk4`（默认）、`euler`、`lu_sgs`、`gmres`、`simplec` 或 `piso`；`simplec` / `piso` 仅用于不可压缩 pressure-velocity 路径。**规划**：`dual_time`（非结构可压缩瞬态隐式，见 [theory/dual_time_stepping.md](theory/dual_time_stepping.md)，**尚未实现**）。**3D 可压缩局部时间步均用 Blazek §6.1.4/§9.1** \(\Delta t_i=\mathrm{CFL}/\sigma_i\)，\(\sigma_i=(\Lambda_i^c+C_v\Lambda_i^v)/V_i\) |
