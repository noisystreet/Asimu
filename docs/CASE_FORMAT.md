...
| `gmres_preconditioner` | 可选；`scalar_diagonal`（默认）、`cell_block_diagonal`（每单元 5×5 局部无粘 Jacobian 块；NS case 叠加粘性抛物对角谱半径）、`lusgs_sweep`（非结构 f64：标量谱半径线性 LU-SGS 双扫，系数取自当前步原始变量与 `[time]` LU-SGS 参数）或 `block_lusgs`（非结构 f64：一阶无粘 5×5 面块 Jacobian 的块 LU-SGS 双扫；NS case 叠加粘性抛物对角与内面近邻近似） |
