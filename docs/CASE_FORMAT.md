...
| `scheme` | 可选；`rk4`（默认）、`euler`、`lu_sgs`、`gmres`、`dual_time`（非结构可压缩瞬态隐式，CPU f32/f64，见 [theory/dual_time_stepping.md](theory/dual_time_stepping.md)）、`simplec` 或 `piso`；`simplec` / `piso` 仅用于不可压缩 pressure-velocity 路径。**3D 可压缩局部时间步均用 Blazek §6.1.4/§9.1** \(\Delta t_i=\mathrm{CFL}/\sigma_i\)，\(\sigma_i=(\Lambda_i^c+C_v\Lambda_i^v)/V_i\) |
...
**规划（`scheme = "dual_time"`，尚未实现）**：`max_inner_steps`（每物理步伪时间迭代上限）、`inner_tolerance`（内层 \(\log_{10}\|R_{\mathrm{eff}}\|_{\mathrm{rms}}\) 早停阈值）。此时 `max_steps` 计物理步，`cfl` 用于伪时间 LTS；须 `local_time_step = true`。**须**与 `[numerics].compute_precision = "f64"|"f32"` 及 `backend = cpu|cuda` 组合使用：f32/f64 双精度自 P0 起同等支持；CUDA 仅 f32（P3b，见 [theory/dual_time_stepping.md](theory/dual_time_stepping.md) §3.5）。详见同文档 §5.2。
