...
| `low_mach_preconditioning` | 可选；启用低马赫预处理（P1 仅非结构可压缩 CPU） |
...
**低马赫预处理（P1）**：字段与语义见 [theory/low_mach_preconditioning.md](theory/low_mach_preconditioning.md)。当前已支持非结构可压缩 CPU 路径；`backend = "cuda"` 暂不支持。
