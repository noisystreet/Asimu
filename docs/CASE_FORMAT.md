...
| `scheme` | 可选；`rk4`（默认）、`euler` 或 `lu_sgs`；**3D 局部时间步均用 Blazek §6.1.4/§9.1** \(\Delta t_i=\mathrm{CFL}/\sigma_i\)，\(\sigma_i=(\Lambda_i^c+C_v\Lambda_i^v)/V_i\) |
| `local_time_step` | 可选；`true` 时逐单元 CFL 时间步（稳态加速；`lu_sgs` **必须**为 `true`） |
| `lusgs_omega` | 可选；`lu_sgs` 松弛因子 \(\omega\in(0,1]\)，默认 1 |
| `lusgs_sweep` | 可选；`false`（默认）仅用对角隐式（阶段 C）；`true` 启用实验性 i/j/k 双扫（阶段 D） |
