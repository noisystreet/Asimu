...
| 时间推进 | `euler`、`rk4`；`lu_sgs`（对角或 `lusgs_sweep`，须 `local_time_step = true`）；`gmres`（matrix-free，须 `local_time_step = true`；非结构 CPU f64 支持 `scalar_diagonal` / `cell_block_diagonal` / `lusgs_sweep` / `block_lusgs`；块预条件须 `reconstruction = first_order`）；非结构 `dual_time`（CPU f32/f64；CUDA 仅 f32） |
...
| `scheme` | 可选；`rk4`（默认）、`euler`、`lu_sgs`、`gmres`、`dual_time`（非结构可压缩瞬态隐式，CPU f32/f64；CUDA 仅 f32，见 [theory/dual_time_stepping.md](theory/dual_time_stepping.md) §3.5）、`simplec` 或 `piso`；`simplec` / `piso` 仅用于不可压缩 pressure-velocity 路径。**3D 可压缩局部时间步均用 Blazek §6.1.4/§9.1** \(\Delta t_i=\mathrm{CFL}/\sigma_i\)，\(\sigma_i=(\Lambda_i^c+C_v\Lambda_i^v)/V_i\) |
