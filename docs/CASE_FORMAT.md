...
# lusgs_sweep = true  # 可选；true=阶段D双扫（默认），false=阶段C对角隐式
# dt = 1.0e-3
# cfl = 0.4
# final_time = 0.2
...
| `scheme` | 可选；`rk4`（默认）、`euler` 或 `lu_sgs`；**3D 局部时间步均用 Blazek §9.1** \(\Delta t_i=\mathrm{CFL}\,V_i/\sigma_i\) |
| `local_time_step` | 可选；`true` 时逐单元 CFL 时间步（稳态加速；`lu_sgs` **必须**为 `true`） |
| `lusgs_omega` | 可选；`lu_sgs` 松弛因子 \(\omega\in(0,1]\)，默认 1 |
| `lusgs_sweep` | 可选；`true`（默认）启用 i/j/k 双扫；`false` 仅用对角隐式（阶段 C） |
