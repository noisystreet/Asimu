...
不可压缩求解内部使用无量纲星号量：\(x^*=x/L_{\mathrm{ref}}\)、\(u^*=u/U_{\mathrm{ref}}\)、\(p^*=p/(\rho U_{\mathrm{ref}}^2)\)、\(\nu^*=1/Re\)。`[incompressible.reference]` 必填，不自动从网格或速度猜测。默认 CGNS Vertex 输出字段为 `Pressure`、`VelocityX`、`VelocityY`、`VelocityZ`，输出会还原 SI。`[incompressible]` 不能与 `[sod]`、`[euler]`、`[navier_stokes]` 同时出现。
