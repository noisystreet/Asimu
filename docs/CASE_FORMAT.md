...
| `backend` | string | `cpu`（默认）或 `cuda`。`cuda` 须以 `--features cuda` 编译，并在 Validate 阶段校验能力矩阵（ADR 0017）；非结构 f32 支持一阶 Euler/RK4、`lu_sgs`、`dual_time`（后者须 `lusgs_sweep = false`） |
...
| 时间推进 | `euler`、`rk4`；`lu_sgs`（对角，须 `local_time_step = true`）；非结构 `dual_time`（CPU f32/f64；CUDA 仅 f32，须 `lusgs_sweep = false`）；结构化另支持 `gmres` |
