...
- 不可压缩 I1 基础算子：结构化 3D `compute_incompressible_divergence_3d`、`compute_incompressible_velocity_laplacian_3d`、压力校正 Poisson CSR 装配骨架与显式 `[incompressible.reference]` 无量纲化；不可压缩 runner 接入 `max|div(u)|` 诊断、压力校正系统装配与 GMRES 求解，并补充理论映射。
