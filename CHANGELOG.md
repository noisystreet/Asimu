...
- 不可压缩 I1 基础算子：结构化 3D `compute_incompressible_divergence_3d`、`compute_incompressible_velocity_laplacian_3d`、瞬态 Stokes 动量预测 CSR 骨架、压力校正 Poisson CSR 装配骨架与显式 `[incompressible.reference]` 无量纲化；不可压缩 runner 接入 `max|div(u)|`、`max|div(u*)|`、动量预测三分量 GMRES 求解诊断、由预测散度驱动的压力校正 GMRES 求解与一次 \(p,\mathbf{u}\) skeleton 修正诊断，并补充理论映射。
