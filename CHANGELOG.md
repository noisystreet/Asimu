...
- ADR 0016 P5：`exec::scatter` 新增 `f32` 原子累加（`AtomicU32` CAS）、`scatter_inviscid_pairs_f32` / `scatter_viscous_valid_slots_f32`；非结构 typed 无粘内面在 `parallel-fvm` 下走着色桶 exec scatter；`unstructured_freestream` benchmark 文档补充 `f32` vs `f64` 对比说明。
