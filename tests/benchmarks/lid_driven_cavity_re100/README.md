...
当前判据要求 100 步 SIMPLEC 外层稳定完成，压力校正与动量线性求解收敛，并把欠松弛后连续性残差压到 `3.0e-5` 以内；SIMPLEC 外层仍使用 `time.tolerance = 3.0e-5` 同时检查连续性、动量残差与速度更新量，因此该粗网格 case 预计 `simplec_converged=false`，用于暴露“连续性已达标但速度更新量仍略高”的状态。
