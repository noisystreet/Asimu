...
pub fn write_compressible_3d_outputs(
    ...
    if let Some(name) = &output.solution_cgns {
        ...
        write_solution_cgns(&path, mesh, fields, eos, physical_time)?;
...
pub fn maybe_write_flow_snapshot(
    ...
    let path = resolve_case_output_path(case.case_dir.as_deref(), &output.dir, &name)?;
    write_solution_cgns(&path, mesh, fields, eos, info.physical_time)?;
    info!(
        path = %path.display(),
        step = info.step,
...
fn write_solution_cgns(
    ...
    #[cfg(feature = "io-cgns")]
    {
        write_flow_cgns(path, mesh, fields, eos, physical_time)
    }
    #[cfg(not(feature = "io-cgns"))]
    {
        let _ = (path, mesh, fields, eos, physical_time);
