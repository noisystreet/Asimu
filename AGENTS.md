...
| `network` 请求默认库（如 `reqwest`） | 与离线 CFD 工作流不符 |

**已批准可选依赖**（须通过 Cargo feature 启用）：`quick-xml`、`base64`、`flate2`（feature `io-vtk`，ADR 0007）；系统 **`libcgns`**（feature `io-cgns`，ADR 0008，无 crates.io 依赖）。
